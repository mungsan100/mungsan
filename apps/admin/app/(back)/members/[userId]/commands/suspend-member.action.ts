...
// 이용 정지 — 활성(정지·탈퇴·삭제 아님) 회원만. 사전조건을 where 에 실은 updateMany 로
// 중복 처리 레이스를 DB 에서 닫는다(승인/반려 패턴). 같은 트랜잭션에서 전 세션을 파기해
// 로그인 중인 기기도 즉시 차단한다(다음 요청부터 middleware 가 /pending 정지 안내로 보냄).
export async function suspendMemberAction(cmd: { userId: string }): Promise<ActionResult> {
...
// 정지 해제 — 정지 상태인 회원만. suspendedAt null 복원(세션은 회원이 다시 로그인해야 생긴다).
